...
class GFObjectPose(nn.Module):
    ...
    def extract_pts_feature(self, data):
        ...
        if self.cfg.dino == "pointwise":
            if "pointwise_rgb_feat" in data and data["pointwise_rgb_feat"] is not None:
                rgb_feat = data["pointwise_rgb_feat"]
            else:
                roi_rgb = data["roi_rgb"]
                feat = self.dino.get_intermediate_layers(roi_rgb)[0]
                token_grid = roi_rgb.shape[-1] // 14
                xs = data["roi_xs"] // 14
                ys = data["roi_ys"] // 14
    ...
    def forward(self, data, mode="score", init_x=None, T0=None):
        ...
        if mode == "score":
            ...
            if "rgb_feat" in data and data["rgb_feat"] is not None:
                ...
                expected_dim = self.dino_dim + self.embedding_dim
                if rgb_feat.shape[-1] == self.dino_dim:
                    return torch.concat(
                        [
                            rgb_feat,
                            encode_axes(
                                data["roi_center_dir"], dim=self.embedding_dim // 6
                            ),
                        ],
                        dim=-1,
                    )
                assert rgb_feat.shape[-1] == expected_dim
                return rgb_feat
