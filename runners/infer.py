...
from datasets.datasets_infer_frontend import FrontendInferDataset
from datasets.types import RGBDFrame
from networks.rgb_frontend import ProvidedMaskFrontend, RGBDInstanceFrontend
...
class GenPose2:
    ...
    def _inference_scale(
        self, data, scale_agent: PoseNet, all_score_feature, all_aggregated_pose
    ):
    ...
    def visualize_pose_distribution(self, data, all_pred_pose):
        for i, test_batch in enumerate([data.get_objects()]):
    ...
    def inference(self, data, prev_pose=None, tracking=False, tracking_T0=0.15):
...
class FrontendGenPose2:
    def __init__(
        self,
        score_model_path: str,
        energy_model_path: str,
        scale_model_path: str,
        frontend: RGBDInstanceFrontend | None = None,
        builder: PoseInstanceBuilder | None = None,
    ):
        self.pose_backend = create_genpose2(
            score_model_path, energy_model_path, scale_model_path
        )
    ...
    def build_dataset_from_raw(self, data: dict) -> FrontendInferDataset:
        return FrontendInferDataset.from_raw_frame(
            data=data,
    ...
    def inference_from_frame(
        ...
        dataset = FrontendInferDataset(
            frame=frame,
            frontend_output=self.frontend.predict(frame, data=raw_data),
            builder=self.builder,
            device=self.pose_backend.cfg.device,
        )
        pose, length = self.pose_backend.inference(
...
def visualize_pose(
    data, all_final_pose, all_final_length, visualize_pts=False, visualize_image=False
):
