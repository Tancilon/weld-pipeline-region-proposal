...
def test_build_mask_candidates_computes_depth_metrics_and_sorts(tmp_path):
    ...
    assert first["coordinate_space"] == "original_image"
    assert first["mask_coordinate_space"] == "original_image"
    assert first["crop_bbox_xywh"] is None
    assert first["roi_bbox_xywh"] is None
